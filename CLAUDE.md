...
- `MCAS Auth Recovery.user.js` — unrelated to the bulk deleters; recovers Teams/Outlook stuck on the boot spinner behind the `*.mcas.ms` Defender for Cloud Apps proxy. Detection is two-factor (dead app shell AND MSAL auth-failure evidence in localStorage) because either alone is a false positive; recovery clears only the stale sign-in markers and navigates to the canonical origin, never `location.reload()` (reloading re-runs the silent-auth path that already failed). Rate-limited to 2 attempts / 10 min per origin.
